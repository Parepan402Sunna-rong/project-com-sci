...
import 'package:weatherlora/modelDate/temDate.dart';
import 'package:weatherlora/modelHour/temHour.dart';
import 'package:http/http.dart' as http;
import 'package:intl/intl.dart';
import 'package:weatherlora/modelMonth/temMonth.dart';
...
class _temperature extends State<temperature> {
  ...
  DateTime day;
  String time;
  // String temText = 'อุณหภูมิ';
  // Utf8Codec utf8 = Utf8Codec();

  static List<TemperatureHour> uvhour;
  static List<TemDate> uvdate;
  static List<TemperatureMonth> uvMonth;

  TemperatureHour datastrH;
  TemDate datastrD;
  TemperatureMonth datastrM;
  ...
  _Hour() {
    ...
    features = [];
    strday = [];
    String url = 'http://202.28.34.197/db/Temperature_Date/' + strDate;
    print(strDate);
    print(url);
    http.get(url).then((response) {
      if (response.statusCode == 200) {
        uvhour = temperatureHourFromJson(response.body);
        if (uvhour != null) {
  ...
  _Date() {
    ...
    features = [];
    strday = [];
    String urlpm1 = 'http://202.28.34.197/db/Temperature_month/' + strDate;
    print(strDate);
    print(urlpm1);
    http.get(urlpm1).then((response) {
      if (response.statusCode == 200) {
        uvdate = temDateFromJson(response.body);
        if (uvdate != null) {
          ...
          // print("gggg" + datavalue.length.toString());
          print(strday.length);
          print(datavalue);
          int sum = 0;
  ...
  _Month() {
    ...
    features = [];
    strday = [];
    String url = 'http://202.28.34.197/db/Temperature_month';
    http.get(url).then((response) {
      if (response.statusCode == 200) {
        uvMonth = temperatureMonthFromJson(response.body);
        if (uvMonth != null) {
